...
void init_map(int col,int row,int M, BLOCK map[col][row]){
  ...
  for(i = 0; i < col; i++){         
    for(j = 0; j < row; j++){
      map[i][j].data = 0;          /* all postitions are empty */
      map[i][j].flag = 0;          /* all postitions are not marked */
      map[i][j].open = 0;          /* all postitions are not opened */
    }
  }
...
int mark(int x,int y,int col,int row,BLOCK map[col][row]){
  ...
  if(map[x][y].open == 1) return 0; 
  else map[x][y].flag = 1 - map[x][y].flag;
